...
/**
 * Ein lesender Aufruf, der einen verlorenen Sitzungswettlauf noch einmal
 * versucht.
 *
 * Fuer einen Charakter, dessen Bot gerade laeuft, gibt es genau eine Sitzung im
 * Spiel. Ein einmaliger Aufruf wie `--view` meldet sich also an und nimmt sie
 * dem Bot weg, der Bot meldet sich neu an und nimmt sie zurueck. Wer dabei
 * unterliegt, bekommt `sessionid invalid`. Gemessen: von fuenfzehn Ansichten
 * hintereinander verloren drei.
 *
 * Ein zweiter Versuch reicht, weil der Bot nach seiner Neuanmeldung eine Weile
 * nicht wieder anklopft. Nur fuer Lesendes: was etwas veraendert, darf nicht
 * von selbst ein zweites Mal laufen.
 */
async function readCli(args, opts = {}) {
  try {
    return await runCli(args, opts);
  } catch (err) {
    if (!/sessionid invalid|session is invalid/i.test(err.message)) throw err;
    await new Promise(r => setTimeout(r, 1500));
    return runCli(args, opts);
  }
}

module.exports = { runCli, readCli, buildArgs, safeValue, CLI_PATH };
