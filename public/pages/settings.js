...
const LABELS_DE = {
  ...
  beer_on_events: { label: 'Bier an Event-Tagen kaufen', desc: 'Nutzt die Event-Kaufmenge, solange ein Event läuft, das als pilzwürdig gilt. Ohne diesen Schalter bleibt es bei der normalen Kaufmenge.' },
  combat_calibration_enabled: { label: 'Gelerntes Kampfmodell anwenden', desc: 'Wendet die gelernte Kalibrierung auf Kampfvorhersagen an, statt nur den rohen Simulator zu nutzen. Ohne Supporter-Lizenz kommt kein Modell an und der Schalter bleibt wirkungslos.' },
...
const LABELS_EN = {
  ...
  beer_on_events: { label: 'Buy beer on event days', desc: 'Uses the event purchase amount while an event counts as worth a mushroom. Without this the ordinary amount applies instead.' },
  combat_calibration_enabled: { label: 'Apply the learned fight model', desc: 'Applies the learned calibration to fight predictions instead of using the raw simulator alone. Without a supporter licence no model arrives and the switch does nothing.' },
